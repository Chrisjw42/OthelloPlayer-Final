...
def convert_state_to_tensorformat(state):
	...
	padded.fill(2)
	padded[1:state.shape[0]+1, 1:state.shape[1]+1] = state

	return padded

def _augment_board_states(boardStates, flip, rotate):
	"""
	Oversample the board states by flipping them and rotating. 
	The theory is that board states are independent of direction, and the NN should believe this. 
	"""
	...
	for board in boardStates:
		...
		if rotate:
			for k in [1,2,3,4]:
				tmp = np.rot90(board, k)
				b.append(tmp)
				b2.append(tmp)
		else:
			thisBoards.append(board)
		if flip:
			for i in b:
				b2.append(np.fliplr(tmp))
			augmentedBoardStates.extend(b2)
		else:
			augmentedBoardStates.extend(b)
	return augmentedBoardStates

def get_tensorinputs_and_labels(forceNew = False, flip = False, rotate = False, takeEveryStep = False, howManyStates = 2, folderName = "2per"):
	"""
	...
	"""
	if not os.path.isfile("data/gamedb80.json".format(folderName)):
		print("There is no Game DB")
		_establish_game_database()

	# If the processing has not ben performed, or forcing new
	if not os.path.isfile("data/{}/tensorinputs80.p".format(folderName)) or not os.path.isfile("data/{}/labels80.p".format(folderName)) or forceNew:
		for file in os.listdir("data"):
			...
			try:
				with open("data/{}/{}".format(folderName, file)) as fp:
					db = json.load(fp)
			except:
				print("Failed to load data/{}/{}, it probably was a filder, not a JSON file".format(folderName, file))
				continue
			...
			for i in list(db.keys()):
				...
				for b in boardStates:
					...
					if thisGame["Winner"] == "Black":
						labels.append(1)
						# This is converted to [0, 1] in the one-hot vector

					else:
						# Winner = White
						labels.append(-1)
						# This is converted to [1, 0] in the one-hot vector

				# Only do it once
				#break
			pickle.dump(tensorinputs, open("data/{}/tensorinputs{}.p".format(folderName, n), "wb"))
			pickle.dump(labels, open("data/{}/labels{}.p".format(folderName, n), "wb"))
	else:
		# The processing has already been done, the inputs and labels are pickled. 

		# Get em.
		tensorinputs = []
		labels = []
		for i in range(81):
			# APPEND, as in, don't extend. This will be a multidimensional array
			tensorinputs.append(pickle.load(open("data/{}/tensorinputs{}.p".format(folderName, i), "rb")))
			labels.append(pickle.load(open("data/{}/labels{}.p".format(folderName, i), "rb")))

	print("Returned data points in {} batches".format(len(tensorinputs)))
